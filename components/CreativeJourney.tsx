...
const CreativeCard = ({ image, text, content }: Props) => {
  return (
    <div className='flex items-center justify-center rounded-3xl relative w-96 h-96 border border-teal overflow-hidden p-3 group smooth' style={{ zIndex: 999 }}>
      ...
      <div style={{ zIndex: 1200 }} className='flex flex-col items-center justify-center gap-3 text-center absolute top-0 left-[50%] transform smooth -translate-x-[50%] translate-y-96 group-hover:translate-y-0 backdrop-blur-md h-full w-full bg-white/40'>
        <h1 className='text-2xl font-medium text-teal'>{text}</h1>
        <p className='max-w-72'>{content}</p>
      </div>
      <h1 className='text-2xl font-medium text-teal absolute top-0 h-full w-full text-center flex items-center justify-center backdrop-blur-sm bg-white/60' style={{ zIndex: "1011" }}>{text}</h1>
    </div>
  )
}
...
const CreativeJourney = () => {
  return (
    <div className='max-w-full w-full flex flex-col items-center justify-center bg-center bg-no-repeat bg-cover' style={{ backgroundImage: `url("/images/assets/HomeBg/cover2.jpg")` }}>
      <div className='flex flex-col items-center justify-center bg-white/80 w-full h-full py-10'>
        <div className='flex flex-col items-center justify-center gap-4 text-center max-w-7xl'>
          <h1 className='text-4xl text-teal-700 font-semibold'>Creative Journey of Livestyle Homes</h1>
          <p>At Livestyle Homes, we believe that every space tells a story. Our creative journey is built around bringing that story to life, turning your vision into a stunning reality that reflects your personality, values, and lifestyle. As seasoned interior and exterior designers, we blend aesthetic beauty with functionality, offering a seamless experience from conception to completion.</p>
        </div>
